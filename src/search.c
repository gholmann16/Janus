...
static GtkSourceSearchContext * context = NULL;
static GtkSourceRegion * last = NULL;
...
void search_init(struct Document * document) {
    ...
    context = gtk_source_search_context_new (GTK_SOURCE_BUFFER(document->buffer), NULL);
    ...
    last = gtk_source_region_new(document->buffer);
    GtkTextIter start, end;
    gtk_text_buffer_get_bounds(document->buffer, &start, &end);
    gtk_source_region_add_subregion(last, &start, &end);
}
...
void search(struct Document * document, enum SearchPattern pat) {
    ...
    if (!gtk_source_region_get_bounds(last, &last_start, &last_end))
    ...
    if ((*search_function)(context, &last_search, &match_start, &match_end, NULL)) {
        gtk_text_buffer_select_range(document->buffer, &match_start, &match_end);

        gtk_source_region_subtract_subregion(last, &last_start, &last_end);
        gtk_source_region_add_subregion(last, &match_start, &match_end);
...
void match_case(GtkWidget * self, struct SearchModel * model) {
    gtk_source_search_settings_set_case_sensitive(model->settings, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(self)));
    search(model->document, SEARCH_FORWARD);
}
...
void search_command(GtkWidget * self, struct Document * document) {
    ...
    static struct SearchModel model;
    static GtkWidget * match_case_button;

    if (dialog == NULL) {
        model.document = document;
        model.settings = gtk_source_search_context_get_settings(context);
        model.search_entry = gtk_entry_new();

        dialog = gtk_dialog_new_with_buttons(_("Find"), window, GTK_DIALOG_DESTROY_WITH_PARENT, _("Cancel"), 0, _("Find"), 1, NULL);
        ...
        gtk_box_pack_start(GTK_BOX(box), model.search_entry, 0, 0, 0);

        g_signal_connect(model.search_entry, "activate", G_CALLBACK(search_entry_forward), &model);
        g_signal_connect(model.search_entry, "activate-backward", G_CALLBACK(search_entry_backward), &model);
    ...
    const gchar * text = gtk_source_search_settings_get_search_text(model.settings);
    if (text)
        gtk_entry_set_text(GTK_ENTRY(model.search_entry), text);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(match_case_button), gtk_source_search_settings_get_case_sensitive(model.settings));
...
void replace(struct SearchModel * model, enum SearchPattern direction) {
    ...
    GtkTextIter start, end;
    if(gtk_source_region_get_bounds(last, &start, &end) == FALSE)
        return;

    gchar * text = gtk_text_buffer_get_text(model->document->buffer, &start, &end, TRUE);
    if (text && strcmp(text, search_text) == 0) {
        g_free(text);
        gtk_source_search_context_replace(context, &start, &end, replace_text, strlen(replace_text), NULL);
    }
...
void replace_command(GtkWidget * self, struct Document * document) {
    ...
    if (dialog == NULL) {
        ...
        model.settings = gtk_source_search_context_get_settings(context);
        model.search_entry = gtk_entry_new();
        model.replace_entry = gtk_entry_new();

        dialog = gtk_dialog_new_with_buttons(_("Replace"), window, GTK_DIALOG_DESTROY_WITH_PARENT, NULL, NULL);
        GtkWidget * content = gtk_dialog_get_content_area(GTK_DIALOG(dialog));
        gtk_box_set_spacing(GTK_BOX(content), 10);
        gtk_container_set_border_width(GTK_CONTAINER(content), 10);
        ...
        gtk_grid_attach(grid, model.search_entry, 1, 0, 1, 1);
        gtk_grid_attach(grid, search_button, 2, 0, 1, 1);

        gtk_grid_attach(grid, replace_label, 0, 1, 1, 1);
        gtk_grid_attach(grid, model.replace_entry, 1, 1, 1, 1);
        ...
        g_signal_connect(model.search_entry, "activate", G_CALLBACK(search_entry_forward), &model);
        g_signal_connect(model.search_entry, "activate-backward", G_CALLBACK(search_entry_backward), &model);
        g_signal_connect(model.replace_entry, "activate", G_CALLBACK(replace_entry_forward), &model);
    ...
    if(gtk_source_region_get_bounds(last, NULL, NULL) == FALSE)
        search(document, SEARCH_FORWARD);
